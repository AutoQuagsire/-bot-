 ...
 *
 * 一次 pipeline 读（两次 SPI 事务）。
 * 失败时：读 ERRFL 清除粘性位，返回 AS5047P_ERROR，角度保持上次有效值。
 * 成功时：更新 dev->angle_rad / angle_deg / raw_angle，返回 AS5047P_OK。
 */
 __attribute__((optimize("O2")))
AS5047P_Status AS5047P_GetAngleWithoutTrack(AS5047P_Handle *dev)
{
    uint16_t dummy = 0U, rx = 0U, errfl = 0U;
    HAL_StatusTypeDef st;

    if (dev == NULL) return AS5047P_ERROR;

    /* txn1: send ANGLECOM, discard residual response */
    st = spi_transfer(dev, AS5047P_CMD_READ_ANGLECOM, &dummy);
    if (st != HAL_OK)
        goto fail_recover;

    /* txn2: send ERRFL early, receive ANGLECOM response (angle data)
     * Pipeline mode: rx = response to previous cmd regardless of current tx */
    st = spi_transfer(dev, AS5047P_CMD_READ_ERRFL, &rx);
    if (st != HAL_OK)
    {
        (void)spi_transfer(dev, AS5047P_CMD_READ_NOP, &dummy);
        goto fail_recover;
    }

    if (!frame_parity_ok(rx))
    {
        /* flush ERRFL response already in flight */
        (void)spi_transfer(dev, AS5047P_CMD_READ_NOP, &errfl);
        return AS5047P_ERROR;
    }

    /* txn3: NOP, receive ERRFL response → clears sticky bits unconditionally */
    (void)spi_transfer(dev, AS5047P_CMD_READ_NOP, &errfl);
