...
{
    for (volatile uint8_t i = 0U; i < 20U; i++) { __NOP(); }
}
...
AS5047P_Status AS5047P_GetAngleWithoutTrack(AS5047P_Handle *dev)
{
    ...
    st = pipeline_read(dev, AS5047P_CMD_READ_ANGLECOM, &rx);

    if (st != HAL_OK || !frame_parity_ok(rx))
    {
        /* 读 ERRFL 清除粘性位，避免下一次响应帧 EF 仍为 1 */
             (void)pipeline_read(dev, AS5047P_CMD_READ_ERRFL, &errfl);
        return AS5047P_ERROR;
    } 

    /* EF=1：ERRFL 有未读错误，读一次清零 */
